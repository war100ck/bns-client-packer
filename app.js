...
class App {
    ...
    const filter = argv[0] ? path.normalize(argv[0]).toLowerCase() : null;
    let processed = 0;
    ...
    for (const file of clientList) {
        ...
        if (filter) {
            const isInFilterPath = filePathNormalized.startsWith(filter) || 
                                filePathNormalized.includes(filter.replace(/\//g, '\\'));
            if (!isInFilterPath) {
...
async processDatabase() {
    ...
    for (const fileInfo of fileInfoList) {
        processed++;
        const filePathNormalized = path.normalize(fileInfo.path).toLowerCase();
